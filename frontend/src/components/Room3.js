...
const Room = () => {
  ...
  const deleteGuest = (peer) => {
    let peerConnection = guestsRef.current.find((guest) => guest.peer == peer).peerConnection;
  ...
  const muteLocalSound = () => {
    localVideo.current.srcObject.getAudioTracks()[0].enabled = false;
  };

  const unmuteLocalSound = () => {
    localVideo.current.srcObject.getAudioTracks()[0].enabled = true;
  };

  const muteRemoteSound = (peer) => {
    let guest = guestsRef.current.find((guest) => guest.peer === peer);
    guest.peerConnection.getRemoteStreams()[0].getAudioTracks()[0].enabled = false;
  };

  const unmuteRemoteSound = (peer) => {
    let guest = guestsRef.current.find((guest) => guest.peer === peer);
    guest.peerConnection.getRemoteStreams()[0].getAudioTracks()[0].enabled = true;
  };
