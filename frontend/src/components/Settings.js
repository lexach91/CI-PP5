...
const Settings = () => {
  ...
  const handleMicrophoneChange = (event) => {
    ...
    const microphone = microphones.find(
      (microphone) => microphone.value === event.value
    );
    // stop current stream if exists
  ...
  useEffect(() => {
    ...
    if (user?.microphone_id) {
      const microphone = microphones.find(
        (microphone) => microphone.value === user.microphone_id
      );
      if (microphone) {
    ...
    <UserLayout title="Settings">
      <div className="grid grid-nogutter p-4 md:w-8 md:mx-auto">
        ...
        <div className="col-12 mt-2">
          {currentCamera ? (
            <div
              style={{ width: "90%", aspectRatio: "16/9", margin: "0 auto" }}
              className="flex justify-content-center align-items-center surface-50">
              <video
                style={{
                  width: "100%",
                  height: "100%",
