...
const Room = () => {
  ...
  const wsOnMessage = (event) => {
    ...
    //     //   create offer for each connected peer
    //   Object.entries(connectedPeers).forEach(([peer, receiver_channel_name]) => {
  ...
  const createOffer = (peer, receiver_channel_name) => {
    ...
    setPeerConnections((prev) => [...prev, peer_connection]);
